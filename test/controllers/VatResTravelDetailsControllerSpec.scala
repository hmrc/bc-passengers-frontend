...
class VatResTravelDetailsControllerSpec extends BaseSpec {
  ...
  "Invoking GET .../duty-free" should {
    ...
    "return the duty free view unpopulated if there is no duty free answer in keystore" in new LocalSetup {
      ...
      override lazy val cachedJourneyData = Some(JourneyData(Some("euOnly"), Some(false), Some(false),Some(false), Some(false), isBringingDutyFree = None))
    ...
    "return the duty free view populated if there is a duty free answer already in keystore" in new LocalSetup {
      ...
      override lazy val cachedJourneyData = Some(JourneyData(Some("euOnly"), Some(false),Some(false), Some(false),Some(false), isBringingDutyFree = Some(true)))
  ...
  "Invoking POST .../duty-free" should {
    ...
    "redirect to the goods-bought-inside-and-outside-eu if not bringing duty free and had previously selected both eu and non eu countries" in new LocalSetup {
      ...
      override lazy val cachedJourneyData = Some(JourneyData(Some("both"),Some(false), Some(false),Some(false),Some(false), isBringingDutyFree = None))
  ...
  "Invoking GET .../duty-free-eu" should {
    ...
    "return the interrupt page for passengers coming from EU that have bought EU" in new LocalSetup {
      ...
      override lazy val cachedJourneyData = Some(JourneyData(Some("euOnly"),Some(false),Some(false),Some(false), Some(false), Some(true)))
  ...
  "Invoking GET .../duty-free-mix" should {
    ...
    "return the interrupt page for passengers coming from EU that have bought EU" in new LocalSetup {
      ...
      override lazy val cachedJourneyData = Some(JourneyData(Some("both"), Some(false),Some(false),Some(false),Some(false), Some(true)))
