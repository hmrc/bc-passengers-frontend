...
class AlcoholInputControllerSpec extends BaseSpec {
  ...
  "Calling GET /select-goods/alcohol/.../volume" should {
    ...
    "return a 200 with and not prepopulate the volume value if there is no working instance" in new LocalSetup {
      ...
      val result: Future[Result] = route(app, EnhancedFakeRequest("GET", "/check-tax-on-goods-you-bring-into-the-uk/select-goods/alcohol/beer/volume/iid0")).get
  ...
  "Calling GET /select-goods/alcohol/.../volume/<iid>/update" should {
    "redirect to the input page and add the purchased product to the working instance" in new LocalSetup {
      ...
      val result: Future[Result] = route(app, EnhancedFakeRequest("GET", "/check-tax-on-goods-you-bring-into-the-uk/select-goods/alcohol/beer/volume/iid0/update")).get

      status(result) shouldBe SEE_OTHER
      redirectLocation(result) shouldBe Some("/check-tax-on-goods-you-bring-into-the-uk/select-goods/alcohol/beer/volume/iid0")

      verify(injected[PurchasedProductService], times(1)).makeWorkingInstance(any(), any())(any(), any())
    }
  }

  "Calling GET /select-goods/alcohol/.../country" should {
  ...
  "Calling GET /select-goods/alcohol/.../country/<iid>/update" should {
    ...
    "redirect to the country input page and add the purchased product to the working instance" in new LocalSetup {
      ...
      val result: Future[Result] = route(app, EnhancedFakeRequest("GET", "/check-tax-on-goods-you-bring-into-the-uk/select-goods/alcohol/beer/country/iid0/update")).get
  ...
  "Calling GET /select-goods/alcohol/.../currency" should {
    ...
    "return a 200 with and not prepopulate if there is no currency in the working instance" in new LocalSetup {
      ...
      val result: Future[Result] = route(app, EnhancedFakeRequest("GET", "/check-tax-on-goods-you-bring-into-the-uk/select-goods/alcohol/beer/currency/iid1")).get
      val content: String = contentAsString(result)
      val doc: Document = Jsoup.parse(content)
    ...
    "return a 200 with the currency value populated if there is a working instance" in new LocalSetup {
      ...
      val result: Future[Result] = route(app, EnhancedFakeRequest("GET", "/check-tax-on-goods-you-bring-into-the-uk/select-goods/alcohol/beer/currency/iid0")).get
      val content: String = contentAsString(result)
      val doc: Document = Jsoup.parse(content)
  ...
  "Calling GET /select-goods/alcohol/.../cost" should {
    ...
    "return a 200 with the cost value populated if there is a working instance" in new LocalSetup {
      ...
      val result: Future[Result] = route(app, EnhancedFakeRequest("GET", "/check-tax-on-goods-you-bring-into-the-uk/select-goods/alcohol/beer/cost/iid0")).get
      val content: String = contentAsString(result)
      val doc: Document = Jsoup.parse(content)
  ...
  "Calling GET /select-goods/alcohol/.../start" should {
    ...
    "redirect to the volume input page" in new LocalSetup {
      ...
      val result: Future[Result]= route(app, EnhancedFakeRequest("GET", "/check-tax-on-goods-you-bring-into-the-uk/select-goods/alcohol/cider/start")).get


      status(result) shouldBe SEE_OTHER
      redirectLocation(result).get should fullyMatch regex """^/check-tax-on-goods-you-bring-into-the-uk/select-goods/alcohol/cider/volume/[a-zA-Z0-9]{6}$""".r
    }
  }

  "Calling POST /select-goods/alcohol/beer/volume/<iid>" should {
    ...
    "redirect to the dashboard when the iid already exists" in new LocalSetup {
      ...
      val result: Future[Result] = route(app, EnhancedFakeRequest("POST", "/check-tax-on-goods-you-bring-into-the-uk/select-goods/alcohol/beer/volume/iid1")
    ...
    "redirect to the country input when the iid is not in the cached journey data" in new LocalSetup {
      ...
      status(result) shouldBe SEE_OTHER
      redirectLocation(result) shouldBe Some("/check-tax-on-goods-you-bring-into-the-uk/select-goods/alcohol/beer/country/iid1")
    }
  }

  "Calling POST /select-goods/alcohol/beer/country/<iid>" should {
    ...
    "redirect to the dashboard when the iid already exists" in new LocalSetup {
      ...
      val result: Future[Result] = route(app, EnhancedFakeRequest("POST", "/check-tax-on-goods-you-bring-into-the-uk/select-goods/alcohol/beer/country/iid1")
    ...
    "redirect to the currency input when the iid is not in the cached journey data" in new LocalSetup {
      ...
      status(result) shouldBe SEE_OTHER
      redirectLocation(result) shouldBe Some("/check-tax-on-goods-you-bring-into-the-uk/select-goods/alcohol/beer/currency/iid1")
    }
  }
}
