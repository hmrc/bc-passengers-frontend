...
class DashboardControllerSpec extends BaseSpec {
  ...
  "respond with 200, display the page if all travel details exist & display button's text for Amendment:Calculate change in taxes and duties" in new LocalSetup {
    ...
    doc.getElementsByClass("govuk-button").text() shouldBe "Calculate change in taxes and duties"
  }
  ...
  "respond with 200 and check if line showing foreign currencies accepted is shown on tell-us page " in new LocalSetup {
    ...
    doc
      .getElementById("foreign-currency")
      .text() shouldBe "Our online calculator accepts most foreign currencies and will work out the tax due in British pounds."
                    ...
                    Band(
                      ...
                      List(
                        Item(
                          ...
                          None,
                          Some(5),
                          Calculation("1.00", "7.00", "90000.00", "90000.00"),
                          Metadata(
                            "5 litres cider",
                            "Cider",
                            "1.00",
  ...
  "Calling GET .../tax-due" should {
    "redirect to the calculation done page with exchange rate message not includes if response only includes GBP currency" in new LocalSetup {
      ...
      content     should not include "We use <a href=\"https://www.gov.uk/government/publications/hmrc-exchange-rates-for-2019-monthly\">HMRC’s exchange rates"
    ...
    "redirect to the calculation done page with exchange rate message if response includes non GBP currency" in new LocalSetup {
      ...
      content     should include("href=\"https://www.gov.uk/government/collections/exchange-rates-for-customs-and-vat\"")
      doc.title shouldBe "Tax due on these goods - Check tax on goods you bring into the UK - GOV.UK"
    }
  }
                    ...
                    List(
                      Item(
                        ...
                        Metadata(
                          ...
                          "500.00",
                          DescriptionLabels("label.Xg_of_X", List("200", "label.tobacco.rolling-tobacco")),
                          Currency("GBP", "British Pound (GBP)", Some("GBP"), Nil),
                          Country("GBP", "Barbados", "GBP", isEu = false, isCountry = true, Nil),
                          ExchangeRate("1.20", "2018-10-29"),
                          None
                        ),
                        None,
  ...
  "display the vat,excise and tax exempt flags as No against items for GBNI Journey" in new LocalSetup {
    ...
    val doc: Document        = Jsoup.parse(content)
    val alcoholItem: Element = doc
      .getElementsByClass("alcohol")
      .get(0)
      .getElementsByClass("hmrc-add-to-a-list")
      .get(0)
    val tobaccoItem: Element = doc
      .getElementsByClass("tobacco")
      .get(0)
      .getElementsByClass("hmrc-add-to-a-list")
      .get(0)
    val otherItem: Element   = doc
      .getElementsByClass("other-goods")
      .get(0)
      .getElementsByClass("hmrc-add-to-a-list")
      .get(0)
  ...
  "not display the vat, excise and tax exempt flags as Empty against items for non GBNI Journey" in new LocalSetup {
    ...
    override val cachedJourneyData: Option[JourneyData] =
      Some(travelDetailsJourneyData.copy(purchasedProductInstances = List(alcohol, tobacco, other)))
  ...
  "not display the tax exempt flags against other items for GBNI Journey for UK Residents" in new LocalSetup {
    ...
    override val cachedJourneyData: Option[JourneyData] = Some(
      travelDetailsJourneyData.copy(
        ...
        purchasedProductInstances = List(other)
      )
    )
  ...
  "display old items(amendment journey) for GBNI Journey for UK Residents" in new LocalSetup {
    ...
    val oldAlcohol: PurchasedProductInstance = PurchasedProductInstance(
      ProductPath("alcohol/beer"),
      "iid0",
      Some(1.54332),
      None,
    ...
    val oldTobacco: PurchasedProductInstance = PurchasedProductInstance(
      ProductPath("tobacco/cigarettes"),
      "iid0",
      Some(1.54332),
      Some(20),
    ...
    val oldOther: PurchasedProductInstance   = PurchasedProductInstance(
      ProductPath("other-goods/antiques"),
      "iid1",
      None,
      None,
    ...
    val alcoholItemCheck: Element = alcoholItem.getElementsByClass("hmrc-add-to-a-list").get(0)
    val tobaccoItemCheck: Element = tobaccoItem.getElementsByClass("hmrc-add-to-a-list").get(0)
    val otherItemCheck: Element   = otherItem.getElementsByClass("hmrc-add-to-a-list").get(0)
