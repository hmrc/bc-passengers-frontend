    ...
    "pass if user is coming from a valid previous step and bringingOverAllowance == false" in new GridSetup {
      ...
      forEachInGrid(params) {
        ...
        case List(prevDeclaration: Option[Boolean], euCountryCheck: Option[String], isVatResClaimed: Option[Boolean], isBringingDutyFree: Option[Boolean], bringingOverAllowance: Option[Boolean]) =>
          ...
          jd match {
            case JourneyData(Some(false), Some("nonEuOnly"), _, _, _,_, _, _,_, Some(false), _, _, _, _, _, _, _, _, _, _, _,_, _, _,_,_)
               | JourneyData(Some(false), Some("euOnly"), _, _,_, _,_,Some(false), Some(true), Some(false), _, _, _, _, _, _, _, _, _, _, _,_, _,_, _,_) =>
              status(res) shouldBe OK
            case _ =>
              status(res) shouldBe SEE_OTHER
          }
    ...
    "pass if user is coming from a valid previous step" in new GridSetup {
      ...
      forEachInGrid(params) {
        case List(prevDeclaration: Option[Boolean], euCountryCheck: Option[String], arrivingNICheck: Option[Boolean], bringingOverAllowance: Option[Boolean]) =>
          ...
          jd match {
            case JourneyData(Some(false), Some("euOnly"), Some(false), _, _, _, _, _, _, Some(true), _, _, _, _, _, _, _, _, _, _, _,_, _, _,_,_) //Q2
                 | JourneyData(Some(false), Some("nonEuOnly"), Some(_), _, _, _, _, _, _, Some(true), _, _, _, _, _, _, _, _, _, _, _,_, _,_, _,_) //Q3, NoNeed
            =>
              status(res) shouldBe OK
            case _ =>
              status(res) shouldBe SEE_OTHER
    ...
    "pass if user is coming from PrivateCraftStep and they have answered the private craft question" in new GridSetup {
      ...
      forEachInGrid(params) {
        case List(prevDeclaration: Option[Boolean], euCountryCheck: Option[String], isVatResClaimed: Option[Boolean], isBringingDutyFree: Option[Boolean], bringingOverAllowance: Option[Boolean], privateCraft: Option[Boolean]) =>
          ...
          jd match {
            case
              JourneyData(Some(false), Some("euOnly"), _, _, _, _, _, Some(false), Some(true), Some(false), Some(_), _, _, _, _, _, _, _, _, _, _,_, _,_, _,_) //Q6, NoNeed
              | JourneyData(Some(false), Some("nonEuOnly"), _, _, _, _, _, _, _, Some(false), Some(_), _, _, _, _, _, _, _, _, _, _,_, _,_, _,_) //Q3, NoNeed
              | JourneyData(Some(false), Some("nonEuOnly"), _, _, _, _, _, _, _, Some(true), Some(_), _, _, _, _, _, _, _, _, _, _,_, _,_, _,_) //Q3
              | JourneyData(Some(false), Some("euOnly"), _, _, _, _, _, Some(true), _, _, Some(_), _, _, _, _, _, _, _, _, _, _,_, _, _,_,_) //Q2
              | JourneyData(Some(false), Some("euOnly"), _, _, _, _, _, Some(false), Some(true), Some(true), Some(_), _, _, _, _, _, _, _, _, _, _,_, _, _,_,_) //Q6
            =>
              status(res) shouldBe OK
            case _ =>
              status(res) shouldBe SEE_OTHER
    ...
    "pass if user is coming from Is17OrOverStep and they have answered the 17 or over question" in new GridSetup {
      ...
      forEachInGrid(params) {
        case List(prevDeclaration: Option[Boolean], euCountryCheck: Option[String], isVatResClaimed: Option[Boolean], isBringingDutyFree: Option[Boolean], bringingOverAllowance: Option[Boolean], privateCraft: Option[Boolean], ageOver17: Option[Boolean]) =>
          ...
          jd match {
            case JourneyData(Some(false), Some("euOnly"), _, _, _, _, _, Some(false), Some(true), Some(false), Some(_), Some(_), _, _, _, _, _, _, _, _, _, _, _,_, _,_) //Q6, NoNeed
                 | JourneyData(Some(false), Some("nonEuOnly"), _, _, _, _, _, _, _, Some(false), Some(_), Some(_), _, _, _, _, _, _, _, _, _, _, _, _,_,_) //Q3, NoNeed
                 | JourneyData(Some(false), Some("nonEuOnly"), _, _, _, _, _, _, _, Some(true), Some(_), Some(_), _, _, _, _, _, _, _, _, _, _, _, _,_,_) //Q3
                 | JourneyData(Some(false), Some("euOnly"), _, _, _, _, _, Some(true), _, _, Some(_), Some(_), _, _, _, _, _, _, _, _, _, _, _,_, _,_) //Q2
                 | JourneyData(Some(false), Some("euOnly"), _, _, _, _, _, Some(false), Some(true), Some(true), Some(_), Some(_), _, _, _, _, _, _, _, _, _, _, _,_, _,_) //Q6
            =>
              status(res) shouldBe OK
            case _ =>
              status(res) shouldBe SEE_OTHER
