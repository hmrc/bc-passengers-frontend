...
class StandardBackLinkModelSpec extends BaseSpec {
  ...
  "Going back to previous-declaration" should {
    "happen when on where-goods-bought" in new LocalSetup {
  ...
  "Going back to check-declare-goods-start-page" should {
    "happen when on previous-declaration" in new LocalSetup {
      ...
      m.backLink(context) shouldBe Some(routes.TravelDetailsController.checkDeclareGoodsStartPage().url)
    }
  }
