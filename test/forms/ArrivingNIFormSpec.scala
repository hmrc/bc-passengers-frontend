  ...
  "arrivingNIForm" should {
    ...
    "return error if the user selects invalid value" in {
      ArrivingNIForm
        ...
        .bind(Map("arrivingNI" -> ""))
        .errors
        .exists(_.message == "error.arriving_ni") shouldBe true
    }
    "return error if the user coming from GB and selects No value" in {
      ArrivingNIForm
        .validateForm(Some("greatBritain"))
        .bind(Map("arrivingNI" -> "false"))
        .errors
        .exists(_.message == "error.arriving_gb") shouldBe true
    }
  }
