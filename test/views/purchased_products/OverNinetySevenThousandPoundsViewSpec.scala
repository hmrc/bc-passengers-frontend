...
import views.html.purchased_products.over_ninety_seven_thousand_pounds

class OverNinetySevenThousandPoundsViewSpec extends BaseViewSpec {
  ...
  val viewViaApply: HtmlFormat.Appendable = injected[over_ninety_seven_thousand_pounds].apply(
  ...
  val viewViaRender: HtmlFormat.Appendable = injected[over_ninety_seven_thousand_pounds].render(
    previousDeclaration = true,
    calculatorResponseDto = calculatorResponseDto,
    deltaCalc = Some(calculation),
    oldAllTax = "0.00",
  ...
  "OverNinetySevenThousandPoundsView" when {
    renderViewTest(
