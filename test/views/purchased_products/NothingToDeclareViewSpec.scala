...
import views.html.purchased_products.nothing_to_declare

class NothingToDeclareViewSpec extends BaseViewSpec {
  ...
  val viewViaApply: HtmlFormat.Appendable = injected[nothing_to_declare].apply(
  ...
  val viewViaRender: HtmlFormat.Appendable = injected[nothing_to_declare].render(
    previousDeclaration = true,
    calculatorResponseDto = calculatorResponseDto,
    deltaCalc = Some(calculation),
    oldAllTax = "0.00",
