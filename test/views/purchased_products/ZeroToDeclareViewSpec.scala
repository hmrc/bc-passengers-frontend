...
import views.html.purchased_products.zero_to_declare

class ZeroToDeclareViewSpec extends BaseViewSpec {
  ...
  val viewViaApply: HtmlFormat.Appendable = injected[zero_to_declare].apply(
  ...
  val viewViaRender: HtmlFormat.Appendable = injected[zero_to_declare].render(
    previousDeclaration = true,
    calculatorResponseDto = calculatorResponseDto,
    deltaCalc = Some(calculation),
    oldAllTax = "0.00",
  ...
  val viewViaF: HtmlFormat.Appendable = injected[zero_to_declare].ref.f(
    true,
