...
import views.html.declaration.zero_to_declare_your_goods

class ZeroToDeclareYourGoodsViewSpec extends BaseViewSpec {
  ...
  val viewViaApply: HtmlFormat.Appendable = injected[zero_to_declare_your_goods].apply(
  ...
  val viewViaRender: HtmlFormat.Appendable = injected[zero_to_declare_your_goods].render(
    calculatorResponseDto = calculatorResponseDto,
    hideExchangeRateInfo = true,
    isAmendment = true,
    backLink = None,
  ...
  "ZeroToDeclareYourGoodsView" when {
    renderViewTest(
