...
class SelectProductServiceSpec extends BaseSpec {
  ...
  "Calling addSelectedProductsAsAliases" should {
    ...
    "add selected products setting in keystore when journey data does exist there currently" in new LocalSetup {
      ...
      override val journeyDataInCache = Some( JourneyData(Some("euOnly"), Some(false),Some(false),Some(false), Some(false), Some(false), Some(false), Some(false), Some(true), Nil) )
      ...
      verify(cacheMock, times(1)).store( meq(JourneyData(Some("euOnly"),Some(false), Some(false),Some(false), Some(false), Some(false), Some(false), Some(false), Some(true),
    ...
    "store selected products which are ProductTreeBranches at the start of the list in keystore, in the order they were sent, followed by ProductTreeLeaves" in new LocalSetup {
      ...
      override val journeyDataInCache = Some(JourneyData(None, None,None, None, None, None, None, None, None, List()))
      ...
      verify(cacheMock, times(1)).store( meq(JourneyData(None,None,None, None, None, None, None, None, None,
        List(
  ...
  "Calling removeSelectedProduct" should {
    ...
    "remove the first selected product and update keystore" in new LocalSetup {
      ...
      override val journeyDataInCache = Some(JourneyData(None,None,None, None, None, None, None, None, None,
