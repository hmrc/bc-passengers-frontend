...
class TobaccoInputForm @Inject() (countriesService: CountriesService, currencyService: CurrencyService) {
    ...
    mapping(
      ...
      "weightOrVolume" -> optional(text)
        ...
        .transform[Option[BigDecimal]](
          grams => grams.map(x => BigDecimal(x)),
          kilos => kilos.map(x => BigDecimal("%.2f".format(x * 1000)).toString)
        )
        .verifying("error.max.decimal.places.weight", weightOrVolume => weightOrVolume.fold(true)(x => x.scale <= 2))
        .transform[Option[BigDecimal]](
          grams => grams.map(x => x / 1000),
          kilos => kilos.map(x => BigDecimal("%.2f".format(x * 1000)))
        ),
