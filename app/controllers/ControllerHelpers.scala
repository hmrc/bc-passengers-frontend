...
trait ControllerHelpers extends MessagesBaseController
  ...
  def withDefaults(jd: JourneyData)(block: Option[String] => Option[String] =>  Option[String] => Future[Result])(implicit context: LocalContext): Future[Result] = {
    jd match {
      case JourneyData(_, _, _,_, _,_, _, _, _, _, _, _, _, _, _, _, _, _, _, defaultCountry, defaultOriginCountry, defaultCurrency,_,_, _,_, _, _) => block(defaultCountry)(defaultOriginCountry)(defaultCurrency)
    }
  }
}
