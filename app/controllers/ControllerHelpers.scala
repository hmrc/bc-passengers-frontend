...
trait ControllerHelpers extends MessagesBaseController
  ...
  def requireCalculatorResponse(block: CalculatorResponse => Future[Result])(implicit context: LocalContext, messagesApi: MessagesApi): Future[Result] = {
    ...
    context.getJourneyData match {
      case JourneyData(_, _,_,_, _, _, _, _, _, _, _, _,_, _, _, _, Some(calculatorResponse), _, _, _) => block(calculatorResponse)
      case _ =>
  ...
  def requireTravelDetails(block: => Future[Result])(implicit context: LocalContext, messagesApi: MessagesApi): Future[Result] = {
    ...
    context.getJourneyData match {
      case JourneyData(Some(_), _, _, _, _,_, _, _, _,Some(_), Some(_), _, _, _, _, _, _, _, _, _) if appConfig.isVatResJourneyEnabled => block
      case JourneyData(Some(_), _, _, _, _, _, None, None, _, Some(_), Some(_), _, _, _, _, _, _, _, _, _) if !appConfig.isVatResJourneyEnabled => block
      case _ =>
  ...
  def withDefaults(jd: JourneyData)(block: Option[String] =>  Option[String] => Future[Result])(implicit context: LocalContext): Future[Result] = {
    jd match {
      case JourneyData(_, _,_, _,_, _, _, _, _, _, _, _, _, _, _, _, _, _, defaultCountry, defaultCurrency) => block(defaultCountry)(defaultCurrency)
    }
  }
}
