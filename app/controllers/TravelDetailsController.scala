...
class TravelDetailsController @Inject() (
  ...
  val didYouClaimTaxBack: Action[AnyContent] = didYouClaimTaxBackAction { implicit context =>
    Future.successful {
      context.journeyData match {
        case Some(JourneyData(_, _,_, _, _,_, _,Some(claimedVatRes), _, _, _, _, _, _, _, _, _, _, _, _ , _,_)) =>
  ...
  val dutyFree: Action[AnyContent] = bringingDutyFreeAction { implicit context =>
    Future.successful {
      context.journeyData match {
        case Some(JourneyData(_, _, _,_,_, _, _, _, Some(isBringingDutyFree), _, _, _, _, _, _, _, _, _, _, _ , _,_)) =>
  ...
  val goodsBoughtIntoNI: Action[AnyContent] = goodsBoughtIntoNIAction { implicit context =>
    Future.successful {
      context.journeyData match {
        case Some(JourneyData(_, _, _,_, _ ,_,_, _, _, Some(bringingOverAllowance), _, _, _, _, _, _, _, _, _, _, _,_)) =>
  ...
  val privateTravel: Action[AnyContent] = privateCraftAction { implicit context =>
    Future.successful {
      context.journeyData match {
        case Some(JourneyData(_, _, _,_,_, _,_, _, _, _, Some(pc), _, _, _, _, _, _, _, _, _, _,_)) =>
  ...
  def confirmAge: Action[AnyContent] = is17OrOverAction { implicit context =>
    Future.successful {
      context.journeyData match {
        case Some(JourneyData(_, _, _,_, _,_,_, _, _, _, _, Some(ageOver17), _, _, _, _, _, _, _, _, _,_)) =>
