  ...
  def processTypeOfIdentification: Action[AnyContent] = dashboardAction { implicit context =>
    ...
    boundForm.fold(
      ...
      identificationTypeDto => {
        val updatedPreUserInformation = context.getJourneyData.preUserInformation.map(_.copy(identification = Some(IdentificationForm(identificationTypeDto.identificationType))))
        ...
        Future.successful(
          Ok(
            idForm.identificationType match {
              case "passport" =>
                passport_id_number(
                  IdentificationDto.form.fill(Identification(Option(idForm.identificationType), idForm.identificationNumber.getOrElse(""))),
                  backLinkModel.backLink
                )
              case "euId" =>
                eu_id_number(
...
//        val updatedJourneyData = context.getJourneyData.copy(preUserInformation = userInformation)
//
//        cache.store(updatedJourneyData).map(_ => updatedJourneyData) flatMap { _ =>

  //THIS (down) WILL BE DONE AT YOUR JOURNEY DETAILS PAGE SO WE NEED TO BUILD THE userInformation before submitDeclaration to declarationService
...
//  }
  ...
  def processEnterYourDetails: Action[AnyContent] = dashboardAction { implicit context => Future.successful(InternalServerError(errorTemplate())) }
