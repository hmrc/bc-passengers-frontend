      ...
      enterYourDetailsDto => {
        ...
        userInformationService.storeUserInformation(context.getJourneyData, userInformation) flatMap { _ =>
          ...
          requireCalculatorResponse { calculatorResponse =>
            ...
            declarationService.submitDeclaration(userInformation, calculatorResponse, context.getJourneyData, receiptDateTime, correlationId, context.request.providerId) flatMap {
  ...
  def irishBorder: Action[AnyContent] = publicAction { implicit context =>
    Future.successful {
      context.journeyData match {
        case Some(JourneyData(_, _, _, _,_, _, _, _, _, _, _, _, Some(irishBorder), _, _, _, _, _, _,_,_)) =>
