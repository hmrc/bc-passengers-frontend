...
class OtherGoodsInputController @Inject()(
  ...
  val addCostForm: Form[OtherGoodsDto] = Form(
    mapping(
      "searchTerm" -> optional(text)
        .transform[Option[OtherGoodsSearchItem]](_.flatMap(term => productTreeService.otherGoodsSearchItems.find(_.name == term)), _.map(_.name)),
      "country" -> text
  ...
  def continueForm(implicit context: LocalContext): Form[OtherGoodsDto] = Form(
    mapping(
      "searchTerm" -> optional(text)
        .verifying("error.other_goods_search", _.fold(context.getJourneyData.selectedAliases.nonEmpty){ term => productTreeService.otherGoodsSearchItems.exists(_.name == term) })
        .transform[Option[OtherGoodsSearchItem]](_.flatMap(term => productTreeService.otherGoodsSearchItems.find(_.name == term)), _.map(_.name)),
      "country" -> text
        .verifying("error.country.invalid", name => countriesService.isValidCountryCode(name)),
      "originCountry" -> optional(text),
  ...
  def displayEditForm(iid: String): Action[AnyContent] = dashboardAction { implicit context =>
    requirePurchasedProductInstance(iid) { ppi =>
      OtherGoodsDto.fromPurchasedProductInstance(ppi) match {
          case Some(dto) => Future.successful(Ok( other_goods_input(addCostForm.fill(dto), Some(iid), countriesService.getAllCountries, countriesService.getAllCountriesAndEu, currencyService.getAllCurrencies, context.getJourneyData.euCountryCheck, productTreeService.otherGoodsSearchItems, "edit", ppi.path) ) )
  ...
  val processAddForm: Action[AnyContent] = dashboardAction { implicit context =>
      def processContinue = continueForm.bindFromRequest.fold(
        formWithErrors => {
          Future.successful(BadRequest( other_goods_input(formWithErrors, None, countriesService.getAllCountries, countriesService.getAllCountriesAndEu, currencyService.getAllCurrencies, context.getJourneyData.euCountryCheck, productTreeService.otherGoodsSearchItems, "create", ProductPath.apply(Nil)) ))
        },
        dto => {
          requireProduct(dto.searchTerm.get.path) { _=>
            val jd = newPurchaseService.insertPurchases(dto.searchTerm.get.path, None, None, dto.country, dto.originCountry, dto.currency, List(dto.cost), dto.searchTerm)
            cache.store(jd._1) map { _ =>
              (context.getJourneyData.arrivingNICheck, context.getJourneyData.euCountryCheck) match {
                case (Some(true), Some("greatBritain")) => Redirect(routes.UKVatPaidController.loadItemUKVatPaidPage(dto.searchTerm.get.path, jd._2))
    ...
    requirePurchasedProductInstance(iid) { ppi =>
      requireProduct(ppi.path) { _ =>
        ...
        def processContinue = addCostForm.bindFromRequest.fold(
          formWithErrors => Future.successful(BadRequest(other_goods_input(formWithErrors, Some(iid), countriesService.getAllCountries, countriesService.getAllCountriesAndEu, currencyService.getAllCurrencies, context.getJourneyData.euCountryCheck, productTreeService.otherGoodsSearchItems, "edit", ppi.path))),
          dto => {
            val jd = newPurchaseService.updatePurchase(ppi.path, iid, None, None, dto.country, dto.originCountry, dto.currency, dto.cost, ppi.searchTerm)
