...
case object WhereGoodsBoughtAmendmentStep
    extends JourneyStep(
      preceeding = List(PreviousDeclarationStep),
...
case object BringingDutyFreeEuStep
    extends JourneyStep(
      preceeding = List(DidYouClaimTaxBackEuOnlyStep),
...
case object DeclareDutyFreeEuStep
    extends JourneyStep(
      preceeding = List(BringingDutyFreeEuStep),
      predicate = _ => _.flatMap(_.isBringingDutyFree).contains(true)
    )

case object BringingDutyFreeBothStep
    extends JourneyStep(
      preceeding = List(DidYouClaimTaxBackBothStep),
      predicate = _ => _.flatMap(_.isVatResClaimed).contains(false)
    )

case object DeclareDutyFreeMixStep
    extends JourneyStep(
      preceeding = List(BringingDutyFreeBothStep),
      predicate = _ => _.flatMap(_.isBringingDutyFree).contains(true)
    )

case object NoNeedToUseStep
    extends JourneyStep(
      preceeding = List(GoodsBoughtIntoGBStep, GoodsBoughtIntoNIStep, DeclareDutyFreeMixStep, DeclareDutyFreeEuStep),
      predicate = _ => _.flatMap(_.bringingOverAllowance).contains(false)
    )

case object Is17OrOverStep
    extends JourneyStep(preceeding = List(PrivateCraftStep), predicate = _ => _.flatMap(_.privateCraft).isDefined)

case object DashboardStep
    extends JourneyStep(
      preceeding = List(DeclarationRetrievalStep, Is17OrOverStep),
      predicate = _ => _.flatMap(_.ageOver17).isDefined
    )

case object ArrivingNIStep
    extends JourneyStep(preceeding = List(WhereGoodsBoughtStep), predicate = _ => _.flatMap(_.euCountryCheck).isDefined)
...
case object UKExcisePaidStep
    extends JourneyStep(preceeding = List(UKResidentStep), predicate = _ => _.flatMap(_.isUKResident).contains(true))

case object UKExcisePaidItemStep
    extends JourneyStep(
      preceeding = List(UKResidentStep),
      predicate = _ =>
        x => x.flatMap(_.euCountryCheck).contains("greatBritain") && x.flatMap(_.arrivingNICheck).contains(true)
    )
...
case object noNeedToUseServiceGbniStep
    extends JourneyStep(
      preceeding = List(UKExcisePaidStep),
...
case object ZeroDeclarationStep
    extends JourneyStep(preceeding = List(DashboardStep), predicate = _ => _.flatMap(_.userInformation).isDefined)
...
case object declarationNotFoundStep
    extends JourneyStep(
      preceeding = List(DeclarationRetrievalStep),
