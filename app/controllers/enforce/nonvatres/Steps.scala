...
case object NoNeedToUseStep
    extends JourneyStep(
      preceeding = List(GoodsBoughtInAndOutEuStep, GoodsBoughtOutsideEuStep),
      predicate = _ => _.flatMap(_.bringingOverAllowance).contains(false)
    )

case object PrivateCraftStep
    extends JourneyStep(
      preceeding = List(NoNeedToUseStep, GoodsBoughtInAndOutEuStep, GoodsBoughtOutsideEuStep),
...
case object DashboardStep
    extends JourneyStep(preceeding = List(Is17OrOverStep), predicate = _ => _.flatMap(_.ageOver17).isDefined)
