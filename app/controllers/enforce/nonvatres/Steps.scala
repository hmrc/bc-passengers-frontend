...
case object NoNeedToUseStep
    extends JourneyStep(
      preceding = List(GoodsBoughtInAndOutEuStep, GoodsBoughtOutsideEuStep),
      predicate = _ => _.flatMap(_.bringingOverAllowance).contains(false)
    )

case object PrivateCraftStep
    extends JourneyStep(
      preceding = List(NoNeedToUseStep, GoodsBoughtInAndOutEuStep, GoodsBoughtOutsideEuStep),
...
case object DashboardStep
    extends JourneyStep(preceding = List(Is17OrOverStep), predicate = _ => _.flatMap(_.ageOver17).isDefined)
