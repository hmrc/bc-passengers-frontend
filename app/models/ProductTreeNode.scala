...
case class ProductTreeLeaf(token: String, name: String, rateID: String, templateId: String, applicableLimits: List[String]) extends ProductTreeNode {
  ...
  def getDescriptionLabels(purchasedProductInstance: PurchasedProductInstance, long: Boolean): Option[(String, List[String])] = {
    ...
    templateId match {
      ...
      case "tobacco" =>
        for(weightOrVolume <- purchasedProductInstance.weightOrVolume) yield
          ("label.Xg_of_X", List(decimalFormat10.format(weightOrVolume * 1000), name))
