...
object IdentificationTypeDto extends Validators {
  ...
  def fromUserInformation(userInfo: UserInformation): IdentificationTypeDto =
    IdentificationTypeDto(userInfo.identificationType)
}

object IdentificationDto extends Validators {

  def form: Form[Identification] = Form(
    mapping(
...
object EmailAddressDto extends Validators {
  ...
  private def emailAddressConstraint: Constraint[EmailAddressDto] =
    Constraint { model =>
      (model.email, model.confirmEmail) match {
        ...
        case (x, y) if (x.isEmpty && y.nonEmpty) || (x.isEmpty && y.isEmpty) => Invalid(ValidationError(s"error.required.emailAddress.email"))
        case (x, y) if x != y =>
          Invalid(s"error.required.emailAddress.no_match")
  ...
  def form: Form[EmailAddressDto] = Form(
    mapping(
      ...
      "confirmEmail" -> text.verifying(maxLength(132, "confirm_email"))
    )(EmailAddressDto.apply)(EmailAddressDto.unapply).verifying(emailAddressConstraint)
  )
}
...
object YourJourneyDetailsDto extends Validators {
  ...
  def form(declarationTime: LocalDateTime): Form[YourJourneyDetailsDto] = Form(
    ...
    )(YourJourneyDetailsDto.apply)(YourJourneyDetailsDto.unapply)
  )
