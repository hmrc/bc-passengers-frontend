...
object AlcoholDto {
  def fromPurchasedProductInstance(purchasedProductInstance: PurchasedProductInstance): Option[AlcoholDto] = for {
    ...
    cost <- purchasedProductInstance.cost
  } yield AlcoholDto(weightOrVolume, country.code, purchasedProductInstance.originCountry.map(_.code), currency, cost,purchasedProductInstance.isVatPaid, purchasedProductInstance.isExcisePaid, purchasedProductInstance.isCustomPaid, purchasedProductInstance.hasEvidence)
}
...
object TobaccoDto {
  def fromPurchasedProductInstance(purchasedProductInstance: PurchasedProductInstance): Option[TobaccoDto] = for {
    ...
    weightOrVolume = purchasedProductInstance.weightOrVolume
  } yield TobaccoDto(noOfSticks, weightOrVolume, country.code, purchasedProductInstance.originCountry.map(_.code), currency, cost, purchasedProductInstance.isVatPaid, purchasedProductInstance.isExcisePaid, purchasedProductInstance.isCustomPaid, purchasedProductInstance.hasEvidence)
}
  ...
  def form(declarationTime: DateTime): Form[EnterYourDetailsDto] = Form(
    mapping(
      ...
      "dateTimeOfArrival" -> mapping(
        ...
        .verifying("error.5_days", dto => new LocalDate(dto.dateOfArrival).toDateTime(LocalTime.parse(dto.timeOfArrival, DateTimeFormat.forPattern("hh:mm aa"))).withZone(DateTimeZone.UTC).isAfter(declarationTime.withZone(DateTimeZone.UTC).minus(Hours.THREE)))
        .verifying("error.5_days", dto => new LocalDate(dto.dateOfArrival).toDateTime(LocalTime.parse(dto.timeOfArrival, DateTimeFormat.forPattern("hh:mm aa"))).withZone(DateTimeZone.UTC).isBefore(declarationTime.withZone(DateTimeZone.UTC).plus(Days.FIVE)))
