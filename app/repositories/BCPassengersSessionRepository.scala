...
class BCPassengersSessionRepository @Inject()(
  ...
  indexes = Seq(
    IndexModel(ascending("updatedAt"), IndexOptions().name("updated-at-index")
      .expireAfter(3600, TimeUnit.SECONDS))
  )
) {

  def fetch[T](key:String)(implicit hc: HeaderCarrier, rds: Reads[T], executionContext: ExecutionContext):Future[Option[JsObject]] = {
    hc.sessionId match {
      case Some(id) =>
  ...
  def store[T](
    ...
    hc.sessionId match {
      case Some(id) => collection.findOneAndUpdate(equal("_id",id.value),
        ...
        FindOneAndUpdateOptions().upsert(true).returnDocument(ReturnDocument.AFTER)).map(_ => body).head()
